...
const StatusIndicatorBox = styled(GreyRowBox)<{ $status: ServerPowerState | undefined }>`
    ...
    & .status-bar {
        ...
        ${({ $status }) => (!$status || $status === 'offline') ? tw`bg-red-400` : ($status === 'running' ? tw`bg-green-400` : tw`bg-yellow-400`)};
    }
    ...
    return (
        <StatusIndicatorBox as={Link} to={`/server/${server.id}`} className={className} $status={stats?.status}>
            <div css={tw`flex items-center col-span-12 sm:col-span-5 lg:col-span-6`}>
                <div css={tw`mr-4 bg-mcm-500 text-neutral-200 rounded-full p-3`}>
            ...
            <div css={tw`hidden lg:col-span-2 lg:flex ml-4 justify-start h-full`}>
